  ...
  PausableUpgradeable
{
  ...
  function initialize(
    ...
    require(address(_beneficiary) != address(0), "beneficiary is zero address");
   ...
   */
  function setClaimer(address _claimer) external onlyRole(DEFAULT_ADMIN_ROLE) {
    claimer = IClaimer(_claimer);
  }
  ...
  function claimOutstandingBid(uint256 id) external {
    ...
    bool success1 = acceptedToken.transfer(
      bidOutstanding.bidder,
      bidOutstanding.forSalePrice - depositAmount
    );
  ...
  function _increaseAppToBeneficiaryFlowWithCtx(bytes memory ctx, int96 amount)
    private
    returns (bytes memory newCtx)
  {
    ...
    if (flowRate > 0) {
      ...
      newCtx = cfaV1.createFlowWithCtx(ctx, beneficiary, acceptedToken, amount);
    }
  }
  ...
  function _decreaseAppToUserFlowWithCtx(
    ...
    if (flowRate > amount) {
      ...
      newCtx = cfaV1.deleteFlowWithCtx(ctx, address(this), user, acceptedToken);
    }
  }
  ...
  function _claim(
    ...
    bool success = acceptedToken.transferFrom(user, beneficiary, claimPrice);
    require(success, "AuctionSuperApp: Claim payment failed");

    // Process claim
    uint256 licenseId = claimer.claim(user, initialContributionRate, claimData);
   ...
   */
  function _getReclaimerStartTime(uint256 id) internal view returns (uint256) {
  ...
  function _increaseBid(
    ...
    if (license.ownerOf(licenseId) == bidder) {
      ...
      return
        _placeNewBid(_ctx, bidder, increasedFlowRate, licenseId, forSalePrice);
    }
  }
  ...
  function _increaseOwnerBid(
    ...
    if (outstandingBidExists) {
      if (
        (block.timestamp - bidOutstanding.timestamp) >= bidPeriodLengthInSeconds
      ) {
      ...
      if (newBidAmount >= bidOutstanding.contributionRate) {
        ...
        bool success = acceptedToken.transferFrom(user, beneficiary, penalty);
  ...
  function _decreaseBid(
    ...
    if (license.ownerOf(licenseId) == user) {
      ...
      return
        _decreaseOldBid(_ctx, user, decreasedFlowRate, licenseId, forSalePrice);
    }
  }
  ...
  function _decreaseOwnerBid(
    ...
    if (decreasedFlowRate > bid.contributionRate) {
      revert("AuctionSuperApp: Cannot decrease bid beyond contribution rate");
    }
  ...
  function _placeNewBid(
    ...
    bool outstandingBidExists = outstandingBid[licenseId].contributionRate > 0;

    if (outstandingBidExists) {
      revert("AuctionSuperApp: Cannot place another bid with one outstanding");
    }
    ...
    if (bidContributionRate >= currentOwnerBid[licenseId].contributionRate) {
      ...
      revert("AuctionSuperApp: New bid must be higher than current owner");
    }
  }
    ...
    returns (bytes memory newCtx)
  {
    ...
    int96 updatedRate = bidOutstanding.contributionRate - bid.contributionRate;
    int96 bidContributionRate = bidOutstanding.contributionRate;
    ...
    emit BidAccepted(licenseId, oldOwner, bidOutstanding.bidder, depositAmount);
  }
  ...
  function _setOutstandingBid(
    ...
    newCtx = _increaseAppToUserFlowWithCtx(_ctx, bidder, bidContributionRate);
    ...
    returns (bytes memory newCtx)
  {
    ...
    (, int96 flowRate, , ) = cfaV1.cfa.getFlowByID(acceptedToken, _agreementId);
    ...
    returns (bytes memory newCtx)
  {
    ...
    if (originalFlowRate < flowRate) {
      return _onIncreaseUserToApp(_ctx, user, flowRate - originalFlowRate);
    } else {
      return _onDecreaseUserToApp(_ctx, user, originalFlowRate - flowRate);
    }
  }
  ...
  function afterAgreementTerminated(
    ...
    if (isUserToApp) {
      ...
      return _onDecreaseAppToUser(_ctx, user, originalFlowRate - flowRate);
    }
  }
  ...
  modifier onlyExpected(ISuperToken superToken, address agreementClass) {
    require(_isSameToken(superToken), "AuctionSuperApp: not accepted token");
    require(_isCFAv1(agreementClass), "AuctionSuperApp: only CFAv1 supported");
    _;
  }
}
