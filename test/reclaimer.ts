...
describe("Reclaimer", async function () {
  ...
  async function buildContract(): Promise<Reclaimer> {
    ...
    const reclaimerContract: Reclaimer = (await upgrades.deployProxy(
      factory,
      []
    )) as Reclaimer;
  ...
  describe("#claimPrice", async () => {
    ...
    beforeEach(async () => {
      ...
      claimData = ethers.utils.defaultAbiCoder.encode(
        ["uint256", "uint256"],
        [originalForSalePrice, today]
      );
    });

    it("should decay the price until the auctionLength expires", async () => {
      const startPrice = await reclaimer
        .connect(user)
        .claimPrice(await user.getAddress(), 0, claimData);
  ...
  describe("#pause()", async () => {
    ...
    it("should only allow the PAUSE role to pause", async () => {
      const reclaimer = await buildContract();
      await expect(reclaimer.connect(user).pause()).to.be.rejectedWith(
        /AccessControl/
      );
    });
  });
  ...
  describe("#unpause()", async () => {
    ...
    it("should only allow the PAUSE role to pause", async () => {
      await expect(reclaimer.connect(user).unpause()).to.be.rejectedWith(
        /AccessControl/
      );
    });
  });
  ...
  describe("setters", async () => {
    ...
    describe("#setAuctionLength", async () => {
      ...
      it("rejects anyone with an ADMIN role", async () => {
        await expect(
          reclaimer.connect(user).setAuctionLength(1512928335)
        ).to.be.rejectedWith(/AccessControl/);
      });
    });
  });
