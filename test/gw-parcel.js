...
describe("GeoWebParcel", async () => {
  ...
  it("should append to parcel", async () => {
    let geoWebParcel = await buildContract();
    let BUILD_ROLE = await geoWebParcel.BUILD_ROLE();
    ...
    // North
    let buildTx = await geoWebParcel.build(coord, [
      makePathPrefix(1).or(BigNumber.from(0)),
    ]);
    ...
    // North, West
    let newPath = [makePathPrefix(2).or(BigNumber.from(0b1100))];
    ...
    assert.equal(
      parcel.path[0].toString(),
      makePathPrefix(1).or(BigNumber.from(0)).toString(),
  ...
  it("should append to single coordinate parcel", async () => {
    let geoWebParcel = await buildContract();
    let BUILD_ROLE = await geoWebParcel.BUILD_ROLE();
    let MODIFY_ROLE = await geoWebParcel.MODIFY_ROLE();
    ...
    // North, North, West
    let newPath = [makePathPrefix(3).or(BigNumber.from(0b110000))];
    ...
    assert.equal(parcel.path[0].toString(), 0, "Stored path is incorrect");
    assert.equal(
      parcel.path[1].toString(),
      newPath[0].toString(),
      "Stored path is incorrect"
  ...
  it("should traverse a parcel after appending", async () => {
    let geoWebParcel = await buildContract();
    let BUILD_ROLE = await geoWebParcel.BUILD_ROLE();
    let DESTROY_ROLE = await geoWebParcel.DESTROY_ROLE();
    let MODIFY_ROLE = await geoWebParcel.MODIFY_ROLE();
    ...
    let result = await geoWebParcel.availabilityIndex(0, 1);
    assert.equal(result.toString(), 0, "Parcel coordinates were not destroyed");
  });

  it("should not append to parcel if caller does not have MODIFY_ROLE", async () => {
    let geoWebParcel = await buildContract();
    let BUILD_ROLE = await geoWebParcel.BUILD_ROLE();
    let MODIFY_ROLE = await geoWebParcel.MODIFY_ROLE();
    ...
    var err;
    try {
      await geoWebParcel.connect(accounts[1]).append(parcelId, newPath);
    } catch (error) {
      err = error;
    }
