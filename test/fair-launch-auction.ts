...
describe("FairLaunchClaimer", async function () {
  ...
  describe("#claim", async () => {
    ...
    it("requires the CLAIMER role", async () => {
      await expect(
        claimer.connect(user).claim(userArg, initialContributionRate, claimData)
      ).to.be.rejectedWith(/AccessControl/);

      const CLAIM_ROLE = await claimer.CLAIM_ROLE();
      await claimer
        .connect(admin)
        .grantRole(CLAIM_ROLE, await user.getAddress());
      await expect(
        claimer.connect(user).claim(userArg, initialContributionRate, claimData)
      ).to.be.fulfilled;
    });
    ...
    describe("success", async () => {
      ...
      it("calls license.safeMint", async () => {
        await claimer
          .connect(user)
          .claim(userArg, initialContributionRate, claimData);
        expect(fakeLicense.safeMint).to.have.been.calledWith(
          userArg,
          licenseId
        );
      });
      ...
      it("user can claim after auction has ended", async () => {
        await claimer
          .connect(admin)
          .setAuctionStart(getUnixTime(startOfToday()));
  ...
  describe("#claimPrice", async () => {
    ...
    it("should decay the price until the auction ends", async () => {
      const startPrice = await claimer
        .connect(user)
        .claimPrice(await user.getAddress(), 20, fakeAddress);
  ...
  describe("#pause()", async () => {
    ...
    it("should only allow the PAUSE role to pause", async () => {
      const claimer = await buildContract();
      await expect(claimer.connect(user).pause()).to.be.rejectedWith(
        /AccessControl/
      );
    });
  });
  ...
  describe("#unpause()", async () => {
    ...
    it("should only allow the PAUSE role to pause", async () => {
      await expect(claimer.connect(user).unpause()).to.be.rejectedWith(
        /AccessControl/
      );
    });
  });
  ...
  describe("setters", async () => {
    ...
    describe("#setStartingBid", async () => {
      ...
      it("rejects anyone with an ADMIN role", async () => {
        await expect(
          claimer.connect(user).setStartingBid(10)
        ).to.be.rejectedWith(/AccessControl/);
      });
    });
    ...
    describe("#setEndingBid", async () => {
      ...
      it("rejects anyone with an ADMIN role", async () => {
        await expect(claimer.connect(user).setEndingBid(10)).to.be.rejectedWith(
          /AccessControl/
        );
      });
    });
    ...
    describe("#setAuctionStart", async () => {
      ...
      it("rejects anyone with an ADMIN role", async () => {
        await expect(
          claimer.connect(user).setAuctionStart(1512918335)
        ).to.be.rejectedWith(/AccessControl/);
      });
    });
    ...
    describe("#setAuctionEnd", async () => {
      ...
      it("rejects anyone with an ADMIN role", async () => {
        await expect(
          claimer.connect(user).setAuctionStart(1512928335)
        ).to.be.rejectedWith(/AccessControl/);
      });
    });
  });
