...
contract("GeoWebCoordinate", async (accounts) => {
  ...
  it("should traverse north", async () => {
    ...
    let result = await gwCoor.traverse(
      makeCoord(0, 0),
      direction,
      new BN(0),
      new BN(0),
    ...
    assert.equal(result.i_x.toString(10), "0", "X is not correct");

    assert.equal(result.i_y.toString(10), "0", "Y is not correct");

    assert.equal(result.i.toString(10), "16", "I is not correct");
  });
  ...
  it("should traverse south", async () => {
    ...
    assert.equal(result.i.toString(10), "0", "I is not correct");
  });
  ...
  it("should traverse east", async () => {
    ...
    assert.equal(result.i.toString(10), "1", "I is not correct");
  });
  ...
  it("should traverse north into new word", async () => {
    ...
    assert.equal(result.i_y.toString(10), "1", "Y is not correct");

    assert.equal(result.i.toString(10), "8", "I is not correct");
  });
  ...
  it("should traverse south into new word", async () => {
    ...
    assert.equal(result.i.toString(10), "248", "I is not correct");
  });
  ...
  it("should traverse east into new word", async () => {
    ...
    assert.equal(result.i_x.toString(10), "1", "X is not correct");

    assert.equal(result.i_y.toString(10), "0", "Y is not correct");

    assert.equal(result.i.toString(10), "128", "I is not correct");
  });
  ...
  it("should traverse west into new word", async () => {
    ...
    assert.equal(result.i.toString(10), "143", "I is not correct");
  });
  ...
  it("should traverse meridian west -> east", async () => {
    ...
    assert.equal(
      result.destination.toString(2),
      makeCoord(2 ** 19 - 1, 0).toString(2),
      "Destination is not correct"
    );

    assert.equal(
      result.i_x.toString(2),
      new BN((2 ** 19 - 1) / 16).toString(2),
      "X is not correct"
    );

    assert.equal(result.i_y.toString(10), "0", "Y is not correct");

    assert.equal(result.i.toString(10), "15", "I is not correct");
  });
