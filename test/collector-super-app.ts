...
describe("CollectorSuperApp", async () => {
  ...
  async function buildCollectorSuperApp({host, cfa, token, receiver}: {host: any, cfa: any, token: any, receiver: any}) {
    const CollectorSuperApp = await ethers.getContractFactory("CollectorSuperApp");
    const superApp = await CollectorSuperApp.deploy(host, cfa, token, receiver);
