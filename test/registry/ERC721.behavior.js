/* OpenZeppelin https://github.com/OpenZeppelin/openzeppelin-contracts/blob/master/test/token/ERC721/ERC721.behavior.js */
const {
  BN,
  constants,
  expectEvent,
...
function shouldBehaveLikeERC721(errorPrefix) {
  ...
  this.ERC721ReceiverMock = null;

  const owner = () => this.accounts[0];
  const newOwner = () => this.accounts[1];
  const approved = () => this.accounts[2];
  const anotherApproved = () => this.accounts[3];
  const operator = () => this.accounts[4];
  const other = () => this.accounts[5];

  context("with minted tokens", function () {
    beforeEach(async function () {
      this.ERC721ReceiverMock = await hre.ethers.getContractFactory(
        "ERC721ReceiverMock"
      );
